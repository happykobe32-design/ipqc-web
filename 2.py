...
# 建立 Drive service（用 cache_resource 避免重複建立）
@st.cache_resource
def get_drive_service():
    try:
        creds = service_account.Credentials.from_service_account_file(
            "ipqc-access.json", scopes=GDRIVE_SCOPES
        )
        service = build("drive", "v3", credentials=creds)
        return service
    except Exception as e:
        st.error("⚠️ 無法建立 Google Drive 連線（檢查 ipqc-access.json 與憑證）")
        st.exception(e)
        return None

def upload_to_gdrive(local_path, filename, folder_id):
    """
    上傳 local_path 檔案到 Google Drive 指定 folder_id，
    回傳上傳成功的檔案 id，失敗回傳 None。
    """
...
if st.sidebar.checkbox("進入後台上傳新資料"):
    new_inspection = st.sidebar.file_uploader("📄 上傳新的點檢資料", type=["xlsx"])
    if new_inspection:
        with open(INSPECTION_PATH, "wb") as f:
            f.write(new_inspection.read())
    
        # 嘗試上傳到 Google Drive（更新資料資料夾）
        try:
            with st.spinner("🔄 正在同步點檢資料到 Google Drive..."):
                # 檔名會保留本地檔名
                gdrive_fname = os.path.basename(INSPECTION_PATH)
                gfile_id = upload_to_gdrive(INSPECTION_PATH, gdrive_fname, folder_id="19042bbfpvRt8QPWpVPM25vlrw86agBIo")
            if gfile_id:
                st.sidebar.success("✅ 點檢資料已更新並同步到 Google Drive")
            else:
                st.sidebar.warning("⚠️ 本地更新完成，但同步到 Google Drive 失敗（請檢查憑證/權限）")
        except Exception as e:
            st.sidebar.error("❌ 同步發生錯誤，請查看 log")
            st.error(e)
    ...
    new_complaint = st.sidebar.file_uploader("📄 上傳新的客訴資料", type=["xlsx"])
    if new_complaint:
        with open(COMPLAINT_PATH, "wb") as f:
            f.write(new_complaint.read())
    
        try:
            with st.spinner("🔄 正在同步客訴資料到 Google Drive..."):
                gdrive_fname = os.path.basename(COMPLAINT_PATH)
                gfile_id = upload_to_gdrive(COMPLAINT_PATH, gdrive_fname, folder_id="19042bbfpvRt8QPWpVPM25vlrw86agBIo")
            if gfile_id:
                st.sidebar.success("✅ 客訴資料已更新並同步到 Google Drive")
            else:
                st.sidebar.warning("⚠️ 本地更新完成，但同步到 Google Drive 失敗（請檢查憑證/權限）")
        except Exception as e:
            st.sidebar.error("❌ 同步發生錯誤，請查看 log")
...
st.caption(f"📁 資料更新時間：點檢資料（{inspection_time}），客訴資料（{complaint_time}）")
            ...
            with st.form("save_form"):
                ...
                if submitted:
                   ...
                   # ✅ 主管 / 被點檢人員 / 點檢人員 各自獨立欄位
                    ...
                    # 嘗試上傳匯出檔到歷史資料夾（history folder）
                    try:
                        with st.spinner("🔄 正在上傳匯出檔到 Google Drive（歷史資料）..."):
                            uploaded_id = upload_to_gdrive(save_path, filename, folder_id="1DfzSY5zeDaeLlsdWGB78wwxrQazxeX3h")
                        if uploaded_id:
                            st.success("✅ 匯出檔已上傳至 Google Drive（歷史資料）")
                        else:
                            st.warning("⚠️ 匯出檔已儲存於本地，但未能上傳至 Google Drive（歷史資料）")
                    except Exception as e:
                        st.error("❌ 上傳歷史資料時發生錯誤，請查看 log")
                        st.exception(e)
